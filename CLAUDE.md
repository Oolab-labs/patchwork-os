...
- `gate explain <workerId> <classKey> [--limit N] [--json]` — "Why did the worker allow/gate THIS action?" Plain-English rendering of the most recent decision(s) for a worker × action-class, from the local Decision Record (`~/.patchwork/worker_gate_decisions.jsonl`) — no bridge required. `--limit N` shows the N most recent (default 1). `classKey` is `domain:reversibility:blastTier` (e.g. `issue:compensable:high`). Also exposed over HTTP at `GET /gate/decisions?workerId=&classKey=&limit=` for the dashboard.

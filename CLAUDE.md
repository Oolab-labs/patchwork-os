...
- **Attributing a gated decision to its approving human is BLOCKED on per-member authentication, which does not exist.** Durability was one blocker and is now resolved (ADR-0018 / #1245 — `ApprovalQueue` persists to `approval_log.jsonl`; timeouts have been risk-tiered since #1214, low 5 min / medium 1 h / high 4 h, so expiry was never the gap). The remaining blocker is identity itself: the bridge authenticates ONE shared bearer token, and the dashboard session cookie is `v1.<expiresAtMs>.<HMAC>` with no subject field. Every approver is therefore indistinguishable at the auth layer. **Do not resolve this by defaulting the actor to the implicit owner** — that writes a claim about a real person into an audit record on no evidence, which is worse than an absent `actor` (absence already means "nobody recorded this", and is never backfilled). It needs a product decision first: per-member passwords, SSO, or per-member bridge tokens.

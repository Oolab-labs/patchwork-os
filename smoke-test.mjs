...
if (targetPort) {
  ...
  lockFile = lockFiles
    .map((f) => ({
      name: f,
      mtime: readFileSync(path.join(lockDir, f)).length,
    }))
...
ws.on("open", async () => {
  try {
    ...
    check(
      "serverInfo",
      init.result?.serverInfo?.name === "claude-ide-bridge",
      JSON.stringify(init.result?.serverInfo),
    );
    check(
      "no false prompts capability",
      init.result?.capabilities?.prompts === undefined,
    );
    check(
      "no false logging capability",
      init.result?.capabilities?.logging === undefined,
    );
    ...
    check(
      ...
      searchTool?.inputSchema?.additionalProperties === false,
    );
    ...
    check(
      ...
      fmtTool?.annotations?.destructiveHint === true,
    );

    // 4. Tool calls
    console.log("\n=== Tool Calls ===");

    const caps = await send("tools/call", {
      name: "getToolCapabilities",
      arguments: {},
    });
    if (caps.error) {
      console.log("  getToolCapabilities error:", caps.error.message);
    }
    const capsData = caps.result
      ? JSON.parse(caps.result?.content?.[0]?.text || "{}")
      : {};
    check(
      "getToolCapabilities",
      capsData.extensionConnected !== undefined,
      `extensionConnected=${capsData.extensionConnected}, features=${JSON.stringify(capsData.features).slice(0, 80)}`,
    );
    ...
    for (const [toolName, toolArgs, validator] of [
      [
        "getGitStatus",
        {},
        (d) => d.branch !== undefined || d.available === false,
      ],
    ...
    // 5. Error handling
    console.log("\n=== Error Handling ===");
    const badTool = await send("tools/call", {
      name: "nonexistent_tool",
      arguments: {},
    });
    check(
      "invalid tool returns -32602",
      badTool.error?.code === -32602,
      `code=${badTool.error?.code}`,
    );
    ...
    check(
      ...
      escape.error !== undefined ||
        JSON.parse(escape.result?.content?.[0]?.text || "{}").files?.length ===
          0,
    );

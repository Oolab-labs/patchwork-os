...
describe("McpTransport", () => {
  ...
  it("extensionRequired tools always visible in tools/list; dispatch returns error when extension disconnected", async () => {
    ...
    send(ws, {
      ...
      id: 2,
      method: "tools/call",
      params: { name: "extensionOnly", arguments: {} },
    });
    const callResp = await waitFor(ws, (m) => m.id === 2);
    ...
    const callResp2 = await waitFor(ws, (m) => m.id === 4);
    expect(callResp2.error).toBeUndefined();
    expect(
      (
        callResp2.result as {
          content: Array<{ type: string; text: string }>;

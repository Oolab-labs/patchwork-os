...
- [ADR-0025: The Evidence Spine Is a Constraint, Not a Subsystem](0025-evidence-spine.md) — six sound ledgers that mostly do not join: 1 run in the whole history is reachable from more than one. Records the principle (absence is never backfilled; design the sentinel, then stamp; `taskId` never `seq`; do not build readers ahead of evidence) separately from the measurements, because it had lived only in `CLAUDE.md` and went stale three times. `approval_log` at 0 of 215 named as the next stamp and why it is the commercially binding one; `outcome-log`, `worker_trust`, `permission_exercises` and per-row hash chaining each left open with the reason
- [ADR-0026: The governed profile and the effective-policy calculation](0026-governed-profile.md) — one `profile: governed | compat` key that resolves to the EXISTING primitives (approval floor, automated-trigger gating, worker authority, agent containment, plugin allowlist, fail-closed kill switch, value-based redaction, one SSRF guard, untrusted envelope); one `computeEffectivePolicy` called by both runners and by `patchwork policy explain`, pinned equal by a runner-level matrix test; `patchwork doctor` reports GOVERNED / NOT GOVERNED from runtime-effective state. Compat is byte-identical to before; only a NEW init starts governed. Remaining bypasses listed, not hidden

...
**Slim mode:** If the bridge started with `--slim`, only ~61 tools are registered (all LSP/editor tools, no shell/git/orchestration). Extension connection still enables LSP functionality within that set. Full mode (the default since v2.43.0) registers 180 tools.

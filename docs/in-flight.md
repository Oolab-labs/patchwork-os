...
- 2026-07-03 `feat/dashboard-humane-w2-roster` — dashboard humane-redesign slice W2 (workers Band 3): restructured the `WorkerCard` mega-card into a **compact roster row** (initial-avatar, name, status chip [★ Ready for a promotion / Proving itself / ⚪ New], one-line job, a compact `RosterBar` 5-stop progress bar with a 🔒 "your limit" marker, and a one-line standing) that **expands to a drawer** holding the full detail (JourneyStepper, "How it got here" timeline, per-task record, ramp-vs-gate). Nothing deleted — the card content just relocated into the drawer. Team of many workers now scannable at the row level. Spec §4 Band 3. Next: W3 (promotion modal + PATCH /workers/:id) / R2 (recipe activity story). — build session

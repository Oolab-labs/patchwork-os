...
- 2026-09-23 `fix/fs-write-rollbackability` — fs-write is reversible only with a CONFIRMED rollback pre-image, assessed before the worker gate / governed profile / policy explain and re-checked before the write (refuse if it worsened). Touches the autonomy gate (actionClass, workerGate), both runners, toolFacts, policyExplain, fileRollback. Automated runs have no rollback log, so their file writes now classify irreversible.

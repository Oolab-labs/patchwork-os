...
- 2026-07-03 `feat/dashboard-humane-r1-recipe-header` — dashboard humane-redesign slice R1 (recipe detail Bands 1+2): a `StatusMedallion` status header (tone/title/sentence from a new pure `lib/recipeStatus.ts` deriver) with the schedule in plain words (`humanizeSchedule`) + primary actions inline (Run now / **Pause** [was Disable] / Edit) + page-level `ExpertToggle`; a conditional **Needs you** band (owner-level `haltPhrasing` sentence + one fix button per row — reconnect/connect/resume/see-what-happened); the destructive action moved out of the Controls card into a folded **Danger zone** ("**Delete this recipe**", was "Uninstall"). Summary/Recent/Halts/Doctor/Simulate cards unchanged (fold into details in R2/R3). Spec §3, §7. Next: W1 (workers Bands 1+2). — build session

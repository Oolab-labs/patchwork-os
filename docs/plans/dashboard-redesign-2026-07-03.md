...
4. **Workers → Roster** (W-A) — `app/workers/page.tsx` — ✅ PR #1084 (SVG trust dial replacing the RosterBar strip; 4-seg progress strip; leash sentence; promote strip; DotStrip footer; three stat tiles; queue above grid; expert-mode DetailsFold drawer content unchanged).
5. **Home → Command Deck** (H-C) — `app/page.tsx` — ✅ PR #1085 (dense 12-col bento; needs-attention span 7 + live-now span 5; row 2 — 24h heatmap/vitals/top-recipes leaderboard; quilt hero + kanban removed from landing, FirstRunChecklist untouched).
6. **Marketplace → Storefront** (M-A) — `app/marketplace/page.tsx` — ✅ PR #1086 (featured split hero + "Before you install" facts; horizontal-scroll shelves grouped by real registry signals; compact tiles; search replaces hero+shelves with the flat filtered grid).
7. **Traces → Waterfall** (T-A) — `app/traces/page.tsx` — ✅ already built in an earlier session (tree view default, per-lane `SpanBar` waterfall, color legend matching `TYPE_THEME`, flat view behind toggle, header stats + live-poll pill) — reviewed against spec, no changes needed.

All 7 core pages done.

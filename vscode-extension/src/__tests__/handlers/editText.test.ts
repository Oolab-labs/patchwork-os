...
describe("handleEditText", () => {
  ...
  it("throws when edits is not an array", async () => {
    await expect(handleEditText({ filePath: "/test.ts", edits: "bad" })).rejects.toThrow("must be an array");
  });

  it("throws when edits exceed 1000", async () => {
    const edits = Array.from({ length: 1001 }, () => ({ type: "insert", line: 1, column: 1, text: "x" }));
    await expect(handleEditText({ filePath: "/test.ts", edits })).rejects.toThrow("Maximum 1000");
  });

  it("throws on unknown edit type", async () => {
    await expect(
      handleEditText({ filePath: "/test.ts", edits: [{ type: "unknown", line: 1, column: 1 }] }),
    ).rejects.toThrow("Unknown edit type");
  });

  it("throws when edit is not an object", async () => {
    await expect(handleEditText({ filePath: "/test.ts", edits: ["bad"] })).rejects.toThrow("must be an object");
  });
  ...
  it("saves document when save=true", async () => {
    ...
    const doc = _mockTextDocument({ fsPath: "/test.ts", save });

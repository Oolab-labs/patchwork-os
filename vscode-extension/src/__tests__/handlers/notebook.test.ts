...
function makeMockCell(overrides: Partial<{
  index: number;
  kind: number;
  outputs: any[];
...
describe("getNotebookCells", () => {
  it("returns cells from a notebook", async () => {
    const cells = [makeMockCell({ index: 0 }), makeMockCell({ index: 1, kind: 1 })];
    const notebook = makeMockNotebook(cells);
    vi.mocked(vscode.workspace.openNotebookDocument).mockResolvedValue(notebook as any);

    const { handlers } = setup();
    const result = (await handlers["extension/getNotebookCells"]({ file: "/test.ipynb" })) as any;
  ...
  it("throws when file is missing", async () => {
    const { handlers } = setup();
    await expect(handlers["extension/getNotebookCells"]({})).rejects.toThrow("file is required");
  });
});
...
describe("runNotebookCell", () => {
  it("runs a cell and returns output", async () => {
    ...
    vi.mocked(vscode.workspace.openNotebookDocument).mockResolvedValue(notebook as any);
    vi.mocked(vscode.window.showNotebookDocument).mockResolvedValue(undefined as any);

    // Simulate execution completing immediately (deferred so `timer` is initialized first)
    vi.mocked(vscode.notebooks.onDidChangeNotebookCellExecutionState).mockImplementation((cb) => {
      Promise.resolve().then(() => cb({ cell, state: vscode.NotebookCellExecutionState.Idle } as any));
  ...
  it("throws when file is missing", async () => {
    const { handlers } = setup();
    await expect(handlers["extension/runNotebookCell"]({ cellIndex: 0 })).rejects.toThrow("file is required");
  });

  it("throws when cellIndex is missing", async () => {
    const { handlers } = setup();
    await expect(handlers["extension/runNotebookCell"]({ file: "/test.ipynb" })).rejects.toThrow(
      "cellIndex is required",
    );
  });
  ...
  it("returns error response when cellAt returns null after execution", async () => {
    ...
    await expect(
      handlers["extension/runNotebookCell"]({ file: "/test.ipynb", cellIndex: 0 }),
...
describe("getNotebookOutput", () => {
  ...
  it("throws when cell not found", async () => {
    ...
    const { handlers } = setup();
    await expect(
      handlers["extension/getNotebookOutput"]({ file: "/test.ipynb", cellIndex: 5 }),

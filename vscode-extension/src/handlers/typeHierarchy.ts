...
export async function handleGetTypeHierarchy(params: Record<string, unknown>): Promise<unknown> {
  ...
  if (!items || items.length === 0) {
    return { found: false, message: "No type hierarchy found at this position" };
  }
  ...
  if (direction === "supertypes" || direction === "both") {
    try {
      const parents = await vscode.commands.executeCommand<vscode.TypeHierarchyItem[]>(
  ...
  if (direction === "subtypes" || direction === "both") {
    try {
      const children = await vscode.commands.executeCommand<vscode.TypeHierarchyItem[]>(

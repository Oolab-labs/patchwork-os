...
interface CopilotAction {
  kind: "pause_recipe" | "enable_recipe" | "run_recipe" | "create_recipe" | "create_worker";
  /** Empty string for create_recipe/create_worker — those carry `goal`
   *  instead until a recipe actually exists to name. */
  recipeName: string;
  ...
  sourceText: string;
  /** create_recipe/create_worker only: the raw goal text to generate from. */
  goal?: string;
  /** create_recipe only: the YAML generated by /recipes/generate, once
   *  "running" has produced a result. Presence gates the Install/Edit/
   *  Discard buttons — set once, never re-generated in place (re-running
   *  the card sends a new chat message instead). */
...
export default function HomePage() {
  ...
  // create_worker never auto-saves a manifest — owns/autonomyCeiling need a
  // human's eyes (see parseIntent.ts's module doc). This just hands off to
  // the /workers/new wizard, prefilled with a name derived from the goal.
  function openWorkerWizard(msg: CopilotMessage) {
    const action = msg.action;
    if (!action?.goal) return;
    setCopilotActionStatus(msg.id, "done");
    recordCopilotDecisionTrace(
      action,
      `Opened the worker wizard prefilled from: "${action.goal}"`,
    );
    const name = action.goal.replace(/^(create|build|make|generate|scaffold)\s+a\s+worker\s*/i, "").trim();
    router.push(`/workers/new${name ? `?name=${encodeURIComponent(name)}` : ""}`);
  }

  // create_recipe: generate a YAML draft for the operator to review. Never
  // saves anything by itself — installGeneratedRecipe (below) is the only
  // path that writes a file, and it always lands the recipe disabled.
  async function generateCopilotRecipe(msg: CopilotMessage) {
    const action = msg.action;
    if (!action?.goal) return;
    ...
    try {
      const res = await fetch(apiPath("/api/bridge/recipes/generate"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: action.goal }),
      });
      const data = (await res.json().catch(() => ({}))) as {
        ok?: boolean;
        yaml?: string;
          ...
          m.id === msg.id && m.action
            ? {
                ...
                action: {
                  ...
                  status: "done",
                  ...(data.ok
                    ? { generatedYaml: data.yaml, generatedWarnings: data.warnings }
                    : { generatedError: data.error ?? "Generation failed." }),
                },
              }
            : m,
        ),
          ...
          m.id === msg.id && m.action
            ? {
                ...
                action: {
                  ...m.action,
                  status: "done",
                  generatedError: e instanceof Error ? e.message : String(e),
                },
              }
            : m,
        ),
  ...
  // Install a copilot-drafted recipe. Always saves via the same
  // saveRecipeContent path hand-authored recipes use, then forces it
  // disabled explicitly (never assumes what a fresh save defaults to) —
  // per the plan's "install lands disabled" safety requirement.
  async function installGeneratedRecipe(msg: CopilotMessage) {
    const action = msg.action;
    if (!action?.generatedYaml) return;
    const nameMatch = /^name:\s*["']?([a-z0-9][a-z0-9-]*)["']?/m.exec(action.generatedYaml);
    const name = nameMatch?.[1];
    if (!name) {
      setCopilotMessages((prev) =>
        prev.map((m) =>
          m.id === msg.id && m.action
            ? { ...m, action: { ...m.action, generatedError: "Could not find a name: field in the generated YAML." } }
            : m,
        ),
      );
      return;
    ...
    try {
      const saveRes = await fetch(apiPath(`/api/bridge/recipes/${encodeURIComponent(name)}`), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: action.generatedYaml }),
      });
      const saveResult = (await saveRes.json().catch(() => ({}))) as { ok?: boolean; error?: string };
      if (!saveResult.ok) {
        setCopilotMessages((prev) =>
          prev.map((m) =>
            m.id === msg.id && m.action
              ? { ...m, action: { ...m.action, generatedError: saveResult.error ?? "Save failed." } }
              : m,
          ),
        );
        return;
      ...
      setCopilotMessages((prev) =>
        prev.map((m) =>
          m.id === msg.id && m.action
            ? { ...m, action: { ...m.action, generatedError: e instanceof Error ? e.message : String(e) } }
            : m,
        ),
      );
    }
  ...
  function discardGeneratedRecipe(msg: CopilotMessage) {
    setCopilotMessages((prev) =>
      prev.map((m) =>
        m.id === msg.id && m.action
          ? { ...m, action: { ...m.action, status: "undone" } }
          : m,
      ),
    );
  }
  ...
  async function confirmCopilotAction(msg: CopilotMessage) {
    ...
    if (action.kind === "create_worker") {
      openWorkerWizard(msg);
      return;
    }
    if (action.kind === "create_recipe") {
      await generateCopilotRecipe(msg);
      return;
    }
    setCopilotActionStatus(msg.id, "running");
              ...
              {m.action && (
                ...
                >
                  <div className="td-copilot-act-head">
                    <span
                      className={`td-pill${m.action.kind === "run_recipe" ? " td-pill-accent" : ""}`}
                    >
                      {m.action.kind === "run_recipe"
                        ? "run"
                        : m.action.kind === "create_recipe"
                          ? "draft recipe"
                          : m.action.kind === "create_worker"
                            ? "draft worker"
                            : "lever"}
                    </span>
                    <strong>
                      {m.action.kind === "create_recipe" || m.action.kind === "create_worker"
                        ? m.action.goal
                        : recipeDisplayName(m.action.recipeName)}
                    </strong>
                  ...
                  {m.action.kind === "create_recipe" && m.action.status === "done" && (
                    <>
                      {m.action.generatedError && (
                        <div className="td-pill td-pill-err" style={{ marginTop: 6 }}>
                          {m.action.generatedError}
                        </div>
                      )}
                      {m.action.generatedYaml && (
                        <>
                          <pre
                            className="mono"
                            style={{
                          ...
                          {m.action.generatedWarnings?.map((w, i) => (
                            <div key={i} className="td-pill" style={{ marginTop: 4 }}>
                              ⚠ {w}
                            </div>
                          ))}
                          <div className="td-copilot-act-actions">
                            <button
                              type="button"
                              className="btn sm primary"
                              onClick={() => void installGeneratedRecipe(m)}
                            >
                              Install (disabled)
                            </button>
                            <Link href="/recipes/new" className="btn sm ghost" style={{ textDecoration: "none" }}>
                              Edit in /recipes/new
                            </Link>
                            <button
                              type="button"
                              className="btn sm ghost"
                              onClick={() => discardGeneratedRecipe(m)}
                            >
                              Discard
                            </button>
                          </div>
